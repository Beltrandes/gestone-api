...
public class QuotationPdfService {
    ...
    public void addHeader(Document document, Quotation quotation) throws IOException {
        ...
        table.addCell(new Cell().add(new Paragraph("Orçamento " + quotation.getLocalId()).setFontSize(20f).simulateBold()).add(new Paragraph("Data: " + quotation.getCreatedAt().format(DateTimeFormatter.ofPattern("dd/MM/yyyy")))).add(new Paragraph("Validade: " + quotation.getCreatedAt().plusDays(quotation.getDaysForDue()).format(DateTimeFormatter.ofPattern("dd/MM/yyyy")))).setTextAlignment(TextAlignment.RIGHT).setBorder(Border.NO_BORDER));
